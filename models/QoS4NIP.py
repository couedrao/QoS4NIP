...
vnf_r_usage = np.random.randint(15, 25, size=(2, host_size - 2))
anf_r_usage = np.random.randint(5, 10, size=(2, host_size - 2))
...
ai_edge = np.full((1, 2), False, dtype=bool)
ai = np.concatenate((ai_cloud_fog, ai_edge), axis=0)
# Supported vnf and anf
fi_cloud_fog = np.full((host_nb - 1, host_size - 2), True, dtype=bool)
fi_edge = np.full((1, host_size - 2), False, dtype=bool)
fi = np.concatenate((fi_cloud_fog, fi_edge), axis=0)
penalty = 1000

################### chromosome
# Effect on apps
# effect_on_apps = np.random.randint(-1, 1, size=(host_nb, app_nb, host_size))
...
sha_xtra = 10 * np.ones((host_nb, 1, 1), dtype=int)  # np.random.randint(10, size=(host_nb, 1, 1))
actu_xtra = 2 * np.ones((host_nb, 1, 1), dtype=int)  # np.random.randint(1, 3, size=(host_nb, 1, 1))
acti_xtra = 2 * np.ones((host_nb, 1, 1), dtype=int)  # np.random.randint(1, 3, size=(host_nb, 1, 1))
...
class QoS4NIP_Scheduler(Problem):

    def __init__(self):
        super(QoS4NIP_Scheduler, self).__init__(1, n_obj, n_obj)
    ...
    def evaluate(self, solution):
        # print('Fct and Act eval running ...')
...
def calc_cfct(x):
    ...
    for h in range(0, host_nb):
        ...
        for j in range(0, host_size - 2):
            if h == host_nb - 1:
                cost_h_cpu += anf_r_usage[0][j] * x[h][0][j]
                cost_h_ram += anf_r_usage[1][j] * x[h][0][j]
            else:
