...
vnf_r_usage = np.random.randint(15, 25, size=(2, 5))
...
# print(np.random.randint(14, 15, size=(app_nb, host_nb)))
# lat = np.random.randint(5, 6, size=(app_nb, host_nb))
# thr = np.random.randint(20, 30, size=(app_nb, host_nb))
# Supported Action
ai_cloud_fog = np.full((host_nb - 1, 2), False, dtype=bool)
...
# appx=np.empty((host_nb, 0, 1), dtype=int)
# for i in range(1, app_nb):
#  appx = np.hstack((appx,-slo[i][1] * np.ones((host_nb, 1, 1))))
...
class QoSEFe_Scheduler(Problem):

    def __init__(self):
        n_obj = app_nb * 3 + 2
        super(QoSEFe_Scheduler, self).__init__(1, n_obj, n_obj)
        ...
        # super(PlanFct, self).__init__(1, n_obj, n_obj)
